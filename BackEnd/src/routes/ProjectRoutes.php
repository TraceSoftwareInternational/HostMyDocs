...
$slim->delete('/deleteProject', function (Request $request, Response $response) {
	...
	$project = $project->setName($name);
	$projectVersion = $projectVersion->setNumber($version, true);
	$projectLanguage = $projectLanguage->setName($language, true);
	...
	if (strlen($language) !== 0 && strlen($version) === 0) {
		$response = $response->write('language must be empty when version is empty');
		return $response->withStatus(400);
	}
	...
	if (!$this->get('projectController')->deleteProject($project)) {
